...
> 其余接口（**网口** / 板上那颗 ES8311 codec / 咪头）本工程没用到。
>
> ⚠️ 别被上面那句误导：**音频是做了的** —— 只是不碰板上那颗 ES8311，而是用 **LEDC PWM**
> 从 **J2 第 26 脚（GPIO20）** 输出、外接一级 RC 低通之后接喇叭/功放（§5.14）：
> 没有 codec、没有 I2S、没有 DMA，纯 PWM 占空比 + 官方 audio 框架。
...
> 症状（用户实测）：`tone` 响一下就没了、第二次直接卡死；`wav_play` 只出 0.2 秒声音。
> 最后用"**逻辑分析仪 + gdb 读活内存 + 给程序加诚实诊断**"三件套收干净。

**① 静音真凶：ISR 里的"半块边界"判成了 `>=`**（本工程自己写的 bug）

```c
p++;
if (p * frame >= S31_AUDIO_BLOCK_BYTES) { 补一块; if (p*frame >= TX_BYTES) p = 0; }   // ✗
```
`p` 每拍 +1，于是 **p=1024..2047 每一拍都成立** → 每个采样都通知框架"补一块"（16kHz 次，
而不是 15.6 次/秒）→ 队列被瞬间抽干、框架往缓冲里 memcpy 的**几乎全是它自己 memset 的 0**
→ 引脚全程静音。修法：判"**刚好跨过**"（`==`）。
⚠️ `tone` 之所以听着正常，是因为它的 1 秒缓冲是**周期性**的，被抽干也照样读到有效样本
—— 典型的"被另一个测试掩盖的 bug"。**排查靠三个诚实诊断**（都留在工程里了）：
`onboard -b`（块设备直读带宽）/ `wav_play` 现在会打"**写入阶段 xx ms**"（写入 ≠ 放完！）
/ `sound -s`（采样时钟自检）。

**② 采样时钟不能"追"**：SYSTIMER 的 target 一旦被长临界区拖后（比如读文件系统时
驱动关中断跑 ROM 读，一次几 ms），`target += period` **永远补不回来** → 中断变成
"一停就立刻再触发"的**风暴**（实测一次 6 秒播放里 ISR 跑了 **1015169 次**，正常应 ≈96000）
→ 随后**整个 CLIC 卡死**：音频和 tick 都停在 pending（`INT_RAW=0x3`）、而 `MIE=1`、
`mintthresh=0x1f`、`mintstatus=0`、CLIC `ie=1` —— 门全开着却再也不进中断，系统只剩空闲线程。
修法：迟到就 `now + period` **重新对齐**，绝不相隔追赶。

**③ 框架调用**挪出 ISR：`rt_audio_tx_complete()` 会一路用到内存池/数据队列/自旋锁，
在中断上下文里跑这些是雷。现在 ISR 只做"写占空比 + 排下一次比较 + 放信号量"，
补块交给 `s31_audio_refill_thread`（每 64ms 一次）。

**④ `msc stop` 要把中断源也掐干净**：只关 CLIC 的 IE、却让 DWC2 继续 assert 中断线，
CLIC 里会留一个"pending 但被屏蔽"的源 —— 之后系统会整个卡死。
现在顺序是：DWC2 `GINTMSK=0` + 清 `GINTSTS` → 清 CLIC pending → 关 CLIC IE。

**🔬 测量教训（这次最值钱的一条）**：判断"引脚上有没有音频"**不能看高/低电平的平均值** ——
对称音频的高低均值都是 3200ns（50%），我据此误判了好几轮；要看**占空比的 min/max**
（或对占空比序列做频谱）。最终证据：`wav_play /SINE8.WAV` 抓到的占空比 **11%↔89%**、
频谱峰值 **440 Hz** ✓；`/STAR8.WAV` 抓到 **G4=392 Hz、D4=294 Hz**（乐谱 392.0/293.7 ✓）。
